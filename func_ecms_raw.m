function [cost, state_out, lambda_out, H, output] = func_ecms_raw(action, state_in, lambda_in, para) %[X C H I out] = fcev_pmp(inp,par)
...
% the only difference between power-based PMP formulation and SOC based
% formulation is in the calculation of Hamiltonian cost. This formulation
% can also be used for ECMS implementation
H = cost/2/Timestep*242*1000 - (lambda_in+omega)*BAT.U_nom*ones(1,Num_action).*i_bat; % Hamiltonian function
% derivative lambda

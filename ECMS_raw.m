...
lambda_sup = -2; % the up limit of the searching interval
lambda_inf = -2.5; % the down limit of the searching interval
...
lambda_opt(1) = lambda_initial; % initialize lambda
    for k = 1:L % at each time step, do instantous optimization
        [cost_temp, state_out_temp, lambda_out_temp, H_temp, output_temp] = func_ecms_raw(Net_a, SOC_opt(k), lambda_opt(k), P_demand(k)); %[X C H I out] = fcev_pmp(inp,par)
        ...
        % in ECMS, lambda is not updated
        lambda_opt(k+1) = lambda_initial;% lambda_out_temp(index_temp); % save current optimal lambda
    end
disp('SOC end')
